...
git clone --branch $INPUT_DESTINATION_BRANCH "https://x-access-token:$API_TOKEN_GITHUB@$INPUT_GIT_SERVER/$INPUT_DESTINATION_REPO.git" "$CLONE_DIR"
...
then
  ...
  then
    ...
    ORIGIN_BRANCH="origin/$INPUT_DESTINATION_BRANCH_CREATE_ORIGIN"
  fi
